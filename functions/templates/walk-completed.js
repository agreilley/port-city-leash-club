...
function html(data) {
  ...
  const photoHtml = photoUrls.length
    ? `<p style="margin:20px 0 0;">${photoUrls.map((url) => `<img src="${escapeHtml(url)}" alt="A photo from today's walk" style="max-width:100%;border-radius:8px;display:block;margin-bottom:8px;">`).join('')}</p>`
    : '';
